...
- iOS: Swift Package Manager layout for Xcode 26 / Swift 6.3 — Swift-only plugin SPM target (CocoaPods keeps ObjC shim); `InitializeOptionsParserPackage` lives under `ios/mparticle_flutter_sdk/` so Flutter’s symlinked SPM tree resolves local path dependencies (no sibling `ios/InitializeOptionsParserCore`).
...
- Web: host apps may need `index.html` updated to Flutter **≥ 3.44.0** bootstrap (`flutter_bootstrap.js`) for Wasm builds.
