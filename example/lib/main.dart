...
/// Resolves mobile example credentials: `--dart-define` first, debug placeholders
/// only in [kDebugMode] so release builds do not embed default keys.
String _exampleMobileApiKey() {
  const fromDefine = String.fromEnvironment('MP_API_KEY');
  if (fromDefine.isNotEmpty) {
    return fromDefine;
  }
  return kDebugMode ? 'example-key' : '';
}

String _exampleMobileApiSecret() {
  const fromDefine = String.fromEnvironment('MP_API_SECRET');
  if (fromDefine.isNotEmpty) {
    return fromDefine;
  }
  return kDebugMode ? 'example-secret' : '';
}
    ...
    try {
      if (kIsWeb) {
        ...
        mpInstance = await MparticleFlutterSdk.initialize(
          MparticleOptions(
            apiKey: _exampleMobileApiKey(),
            apiSecret: _exampleMobileApiSecret(),
      ...
      if (kIsWeb && e.code.startsWith('MP_WEB_')) {
        print('mParticle web init failed: ${e.code}: ${e.message}');
      } else {
        rethrow;
      }
    }
          ...
          builder: (context) {
            return ListView(
              children: <Widget>[
                ...
                Center(child: Text('EVENT LOGGING')),
                ...
                buildButton('Alias Users with MPIDs and Times', () {
                  ...
                  mpInstance?.identity.aliasUsers(
                  ...
                  mpInstance?.identity
                      ...
                      .then(
                        (IdentityApiResult successResponse) {
                          String? previousMPID = successResponse.previousUser
                              ?.getMPID();
                          ...
                          var identityRequest =
                              MparticleFlutterSdk.identityRequest;
                          identityRequest
                              .setIdentity(
                                identityType: IdentityType.CustomerId,
                                value: 'customerid2',
                        ...
                        onError: (error) {
                          var failureResponse =
                              error as IdentityAPIErrorResponse;
                ...
                Center(child: Text('NATIVE ONLY METHODS')),
                ...
                buildButton('user - get GDPR Consent State', () async {
                  var user = await mpInstance?.getCurrentUser();
                  Map<String, Consent>? gdprConsent = await user
                      ?.getGDPRConsentState();
                ...
                buildButton(
                  'user - add 2nd approved GDPR Consent State',
                  () async {
                    var user = await mpInstance?.getCurrentUser();
